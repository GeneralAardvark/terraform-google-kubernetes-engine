...
locals {
  ...
  create_cmd_gke_body       = "${local.is_gke_flag} ${var.gke_hub_membership_name} ${var.cluster_uri} ${local.gke_hub_sa_key} ${var.project_id} ${var.labels}"
  destroy_gke_entrypoint    = "${path.module}/scripts/gke_hub_unregister.sh"
  destroy_gke_body          = "${local.is_gke_flag} ${var.gke_hub_membership_name} ${var.cluster_uri} ${var.project_id}"
}
